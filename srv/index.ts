...
export default (app : Router, http : any) => {
    ...
    for(let [path, route] of Object.entries(routes[RouteListSymbol] || {})) {
        const wrappedRoute = async (req : Request, res : Response) => {
            try {
                ...
                args.push(req.query);
                const params = Reflect.getMetadata("design:paramtypes", routes, route.name);
                ...
                const resp = await (routes as any)[route.name].apply(this, args);
